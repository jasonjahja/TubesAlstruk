...
void Save(ListofPenyanyi *LP,const char *filename){
    ...
    for(int i = 0; i < BanyakPenyanyi(*LP);i++){
        fprintf(pita,"%d %s\n",BanyakAlbum(LP->listpenyanyi[i].album),LP->listpenyanyi[i].namaPenyanyi.TabWord);
        for (int j = 0; j < BanyakAlbum(LP->listpenyanyi[i].album);j++){
            fprintf(pita,"%d %s\n",BanyakLagu(LP->listpenyanyi[i].album.listalbum[j].listlagu),LP->listpenyanyi[i].album.listalbum[j].namaAlbum.TabWord);
            for (int k = 0; k < BanyakLagu(LP->listpenyanyi[i].album.listalbum[j].listlagu);k++){
                fprintf(pita,"%s\n",LP->listpenyanyi[i].album.listalbum[j].listlagu.Lagu[k].judul.TabWord);
            }
        }
    }
}
